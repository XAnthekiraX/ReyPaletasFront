...
const philosophy = [
  {
    title: 'Misión',
    description: 'Crear momentos extraordinarios a través de un helado auténtico, divertido y delicioso, llevando la experiencia del helado artesanal a cada rincón del país.',
    icon: 'mdi:target'
  },
  {
    title: 'Visión',
    description: 'Ser la marca preferida de helados artesanales en Ecuador, reconocida por nuestra innovación, calidad y capacidad de transformar lo cotidiano en extraordinario.',
    icon: 'mdi:eye'
  },
  {
    title: 'Valores',
    items: [
      { name: 'Innovación', icon: 'mdi:lightbulb' },
      { name: 'Calidad', icon: 'mdi:star' },
      { name: 'Tradición', icon: 'mdi:heart' },
      { name: 'Cercanía', icon: 'mdi:account-group' }
    ],
...
const allianceIcons = [
  ...
  { name: 'Minimarkets', icon: 'mdi:store' },
...
function TimelineItem({ event, index, isLeft }) {
  return (
    ...
    >
      <div className={`flex ${isLeft ? 'flex-row' : 'flex-row-reverse'} md:flex-row items-center w-full`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          {item.items?.map((val, i) => (
            <div key={i} className="flex items-center gap-2">
              <Icon icon={val.icon} className="w-5 h-5 text-primary" />
